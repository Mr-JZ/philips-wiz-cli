...
class Wiz:
    ...
    def change_color(self):
        ...
        message = (
            '{"id":1,"method":"setPilot","params":{"r":0,"g":0,"b":0,"dimming":100}}'
        )
    ...
    def ask_color(self):
        red = click.prompt(
            colored("Please enter value of red", "red"),
            type=int,
            default=0,
            show_default=True,
        )
        green = click.prompt(
            colored("Please enter value of green", "green"),
            type=int,
            default=0,
            show_default=True,
        )
        blue = click.prompt(
            colored("Please enter value of blue", "blue"),
            type=int,
            default=255,
            show_default=True,
        )
    ...
    def set_scene(self):
        ...
        user_prompt = click.prompt(
            colored("choose an scene:", "yellow"),
            type=int,
            default=1,
            show_default=True,
        )
...
def udp_client(ip, port, message, return_response=False):
    ...
    while True:
        try:
            ...
            print(
                colored(
                    "[-] Looks like given ip is not wiz-light , try looking ip address in your wiz mobile app !",
                    "red",
                )
            )
            exit()
...
def error_check(port, ipaddr):
    ...
    if valid_ip == False:
        print(
            colored(
                "[-] ip address is not valid / checking port is available didnt take place ..",
                "red",
            )
        )
...
@click.command()
@click.option(
    "-p", "--port", default=38899, type=int, help="port number default(38899)"
)
@click.option(
    "-ip",
    "--ipaddr",
    required=True,
    type=str,
    prompt="please enter the ip of your wiz light (you can get it from your wiz mobile app)",
    help="ip address of wiz light",
)
@click.option(
    "-m",
    "--message",
    default="hello world (default message)",
    type=str,
    help="request to send (recommended for advanced users)",
)
...
@click.option(
    "-ss", "--setscene", is_flag=True, help="set a new scene for your wiz light"
)
